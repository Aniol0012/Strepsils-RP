	...
	while true do
		...
		if not Config.OnlyPolicemen or (Config.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police") then
			...
			if dstCheck <= 5.0 then
				...
				local text = "~b~Armería"

				if dstCheck <= 0.5 then
					text = "Pulsa [~g~E~s~] para abrir la ~b~armería"
				...
				elseif dstCheck >= 0.5 and IsMenuOpen then
...
local IsMenuOpen = false -- Hecho por Aniol0012, de esta forma el menú se cierra al salir de las coordenadas.

OpenPoliceArmory = function()
	PlaySoundFrontend(-1, 'BACK', 'HUD_AMMO_SHOP_SOUNDSET', false)

	local elements = {
		{ ["label"] = '<span style="color:green;">🔫 Sacar armas<span style="color:white;">', ["action"] = "weapon_storage" },
		{ ["label"] = '<span style="color:red;">💼 Guardar armas<span style="color:white;">', ["action"] = "weapon_supp" }
	}

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "police_armory_menu",
		{
			title    = '<span style="color:cyan;">👮 Armería policial<span style="color:white;">',
			align    = "right",
			elements = elements
		},
	function(data, menu)
		...
		elseif action == "weapon_supp" then
			...
			ESX.ShowAdvancedNotification('CNP', '~g~Has guardado tus armas', '', 'CHAR_OMEGA', 1)
		end		
...
OpenWeaponStorage = function()
	...
	function(data, menu)
		...
		if (DoesEntityExist(closestPed) and closestPedDst >= 5.0) or IsPedAPlayer(closestPed) then
			RefreshPed(true) -- Esto hace que reaparezca el ped si de alguna forma desaparece.

			ESX.ShowAdvancedNotification('CNP', '~o~Inténtalo de nuevo', '', 'CHAR_OMEGA', 1)
		...
		if IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "pistol_on_counter_cop", 3) or IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "rifle_on_counter_cop", 3) then
			ESX.ShowAdvancedNotification('CNP', '~o~Espera tu turno, por favor', '', 'CHAR_OMEGA', 1)
			return
		end
