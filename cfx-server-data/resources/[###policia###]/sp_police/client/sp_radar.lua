...
function radarSetSpeed(defaultText)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString("~r~Introduce un número correcto.")
...
function POLICE_radar()
    ...
    if isRadarPlaced then 
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString("~r~¡No estás al lado de tu radar!")
            DrawSubtitleTimed(3000, 1)
        ...
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format("   El vehículo ~r~%s ~w~con matrícula ~r~%s ~w~va a ~r~%s ~w~km/h", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format("   Vehículo ~r~%s ~w~con matrícula ~r~%s ~w~va a ~r~%s ~w~km/h", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format("   Vehículo ~r~%s ~w~con matrícula ~r~%s ~w~va a ~r~%s ~w~km/h", LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end
