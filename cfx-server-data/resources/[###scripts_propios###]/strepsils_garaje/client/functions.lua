    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Sacar " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " | Matrícula: " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Aquí no tienes ningún vehículo aparcado."
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " | Matrícula: " .. vehicleData["plate"] .. " | " .. dstCheck .. " metros.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "No tienes ningún vehículo fuera."
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        ...
        {
            ["label"] = " " .. (DoesBlipExist(cachedData["blips"][vehicleEntity]) and "Apagar" or "Encender") .. " el rastreador gps del vehículo",
            ["action"] = "change_gps_state"
        },
        {
            ["label"] = "Controlar puertas",
            ["action"] = "change_door_state"
        },
    }

    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Vehículo:" .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Estás demasiado lejos de tu vehículo para controlarlo") end
    ...
    elseif action == "change_gps_state" then
        if DoesBlipExist(cachedData["blips"][vehicleEntity]) then
            ...
            SetBlipScale(cachedData["blips"][vehicleEntity], 0.7)
            SetBlipColour(cachedData["blips"][vehicleEntity], 2)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Tu vehículo: " .. GetVehicleNumberPlateText(vehicleEntity))
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("~r~Hay un vehículo en el sitio")
...
SpawnVehicle = function(vehicleProps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Este vehículo ya está fuera, no puedes sacar dos veces el mismo vehículo")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("~g~Has sacado tu vehículo del garaje")
		
		ESX.ShowNotification("~o~Pulsa F5 para aceder a las opciones de tu vehículo")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("~g~Has guardado tu vehículo")
			else
				ESX.ShowNotification("~r~Este vehículo no es tuyo.")
			end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return ESX.ShowNotification("~r~Este vehículo no existe")
    end
