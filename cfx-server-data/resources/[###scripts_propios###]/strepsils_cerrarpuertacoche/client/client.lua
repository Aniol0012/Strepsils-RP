                    ...
                    if(string.lower(plate) == localVehPlate)then
                        ...
                        if(vehicle ~= "locked")then
                            ...
                            if(time > timer)then
                                ...
                                TriggerEvent("ls:notify", "Tienes que esperar, ten paciencia..." .. (timer / 1000) .." segundos")
                            end
                        else
                            TriggerEvent("ls:notify", "Las llaves no estan dentro")
                        end
                    end
                end
                ...
                if(not hasKey)then
                    ...
                    if(isInside)then
                        ...
                        if(canSteal())then
                            ...
                            TriggerEvent("ls:notify", "Las llaves no están dentro")
                        end
                    end
                end
            end
...
AddEventHandler("ls:getHasOwner", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    if(not hasOwner)then
        ...
        TriggerEvent("ls:notify", "Este vehículo no te pertenece")
    end
end)
...
-- @param string plate
RegisterNetEvent("ls:darllaves")
AddEventHandler("ls:darllaves", function(plate)
    local plate = string.lower(plate)
...
function Notify(text, duration)
	if(globalConf['CLIENT'].notification)then
		if(globalConf['CLIENT'].notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "LLaves del Coche ", "Strepsils RP", duration)
			DrawNotification_4(false, true)
		elseif(globalConf['CLIENT'].notification == 2)then
			TriggerEvent('chatMessage', '^1Cerrar el coche' .. _VERSION, {255, 255, 255}, text)
		else
			return
		end
	else
