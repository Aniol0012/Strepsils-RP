...
function closeBanking() {
    $.post("http://pw_atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("http://pw_atms/playATMAnim", JSON.stringify({}));
        var selectedCard = $(this).data('card')
        var cardPri = jQuery.parseJSON(selectedCard.metaprivate);
        clientPin = cardPri.pin;
        cardSelected = selectedCard
        clientCid = cardPri.cid;
        cardNumb = cardPri.cardnumber
        $('#cardSelector').css({"display":"none"});
    ...
    $(document).on('click','[data-action=ATMwithdraw]',function(){
        ...
        if(amount !== undefined && amount !== null && amount !== 0) {
            $.post("http://pw_atms/doATMWithdraw", JSON.stringify({
    ...
    $(document).on('click','[data-act=enterNumber]',function(){
            ...
            if (number == "ACCEPT") {
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("http://pw_atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
