...
-- Al desactivar esto, todo lo referente al coche se desactiva (speedometer, seatbelt...)

Citizen.CreateThread(function()
	if Config.ui.showCar == true then
...
ToggleHud2 = function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'blackMoney' })
		SendNUIMessage({ action = 'element', task = 'disable', value = 'wallet' })
		SendNUIMessage({ action = 'element', task = 'disable', value = 'society' }) -- Modificado por Aniol0012: Esto hace que se esconda el society al pedir que lo haga
	else
		if (Config.ui.showJob == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'job' })
		end
...
RegisterNetEvent('strepsils_hud2')
AddEventHandler('strepsils_hud2', function()
	ToggleHud2()
end)

RegisterNetEvent('strepsils_cerrar_hud2')
AddEventHandler('strepsils_cerrar_hud2', function()
	if not toggleui then
		SendNUIMessage({ action = 'element', task = 'disable', value = 'job' })
		SendNUIMessage({ action = 'element', task = 'disable', value = 'society' })
		SendNUIMessage({ action = 'element', task = 'disable', value = 'bank' })
