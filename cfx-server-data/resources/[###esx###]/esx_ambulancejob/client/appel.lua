...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("EMS", "~b~Llamada de EMS", "Alguien necesita un EMS !\n~g~Y~w~ para responder la llamada\n~r~X~w~ para rechazar la llamada", "CHAR_CALL911", 8)
end)
     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				ESX.ShowAdvancedNotification("EMS", "~b~Llamada de EMS", "Has rechazado la llamada.", "CHAR_CALL911", 8)
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("EMS", "~b~Llamada de EMS", "~r~La llamada ya ha sido contestada.", "CHAR_CALL911", 8)
		end
     end
end)
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("EMS", "~b~Llamada de EMS", "Has cogido la llamda, ve a la dirección marcada en el GPS.", "CHAR_CALL911", 8)   
...
function afficherTextVolant(XAcoords, XAid)
	...
	AddTextComponentSubstringPlayerName("~b~Llamada de EMS")
	...
	-- Ajout de la route
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~Aquí hay una persona que necesita ser reanimada ...\nPulsa ~b~[E]~w~ para revivirlo.", 4, 0.1, 0.1)
...
Citizen.CreateThread( function()	
	while true do
		...
		if enService then
			...
			DrawAdvancedText(0.984, 0.214, 0.008, 0.0028, 0.4, "Ultima llamada:", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.988, 0.236, 0.005, 0.0028, 0.4, "~b~"..NomAppel.." ~w~ha sido el último en cojer una llamada de EMS.", 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.984, 0.274, 0.008, 0.0028, 0.4, "Total de llamadas respondidas", 0, 191, 255, 255, 6, 0)
