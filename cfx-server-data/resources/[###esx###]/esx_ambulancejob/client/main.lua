...
Citizen.CreateThread(function()
  while true do
      Citizen.Wait(10 * 180000) -- Media hora
	  --TriggerServerEvent('exp:addExperience',100)
	  if ESX.PlayerData.job and (ESX.PlayerData.job.name == 'admin' or ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job.name == 'taxi' or ESX.PlayerData.job.name == 'mafia1')then
        TriggerServerEvent('exp:addExperience',100)
	  end
  end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsDead then
			...
			EnableControlAction(2, 1, true) -- Disable pan
			EnableControlAction(2, 2, true) -- Disable tilt
		else
			Citizen.Wait(500)
		end
	end
...
function OnPlayerDeath()
	...
	StartScreenEffect('DeathFailOut', 0, false) -- Creo que al activar esto hace que no puedas mirar con la camara y eso...
	
	TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#FFFF'>Has muerto, pulsa [<b style='color:#1EFF29'>G<b style='color:#FFFF'>] para avisar a un médico.</b>", --Has muerto, pulsa [<b style='color:#1EFF29'> G] <b style='color:#FFFF'> para avisar a un médico.</b>
					type = "info",
					timeout = (30000),
					layout = "centerRight",
					queue = "global"
...
AddEventHandler('esx_ambulancejob:notif', function()
	...
	ReaFaite = true
	ESX.ShowAdvancedNotification('EMS', 'Centro de EMS', 'Reanimacion completada. \nSe han añadido ~g~250€~w~ a tu cuenta.\n~g~'..Nombreinter..' intervención en progreso.', 'CHAR_CALL911', 3)
end)
...
function RemoveItemsAfterRPDeath()
	...
	Citizen.CreateThread(function()
		...
		PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
		ESX.ShowAdvancedNotification('Reanimación de EMS', '', 'Has sido revivido por un EMS', 'CHAR_CALL911', 1)
		local ped = GetPlayerPed(PlayerId())
		local coords = GetEntityCoords(ped, false)
		local name = GetPlayerName(PlayerId())
...
AddEventHandler('esx_ambulancejob:NotificationBlipsX2', function(blipId, x, y, z)
	...
	ESX.ShowAdvancedNotification('Información', 'EMS', 'Alguien ha sido por un EMS. \n~g~ Quedan'..Nombreinter..' intervenciones en progreso.', 'CHAR_CALL911', 1)
	...
	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName('Reanimación de EMS')
...
AddEventHandler('esx_ambulancejob:revive2', function()
	...
	Citizen.CreateThread(function()
		...
		ESX.ShowAdvancedNotification('Admin', '', 'Has sido reanimado por un administrador.', 'CHAR_CALL911', 8)
	end)
end)
