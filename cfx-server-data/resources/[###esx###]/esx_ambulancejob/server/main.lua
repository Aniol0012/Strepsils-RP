...
AddEventHandler('AnnounceEMSOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Anuncio del Hospital', '~g~¡Hay un EMS disponible!', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('AnnounceEMSFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Anuncio del Hospital', '~r~¡No hay ningún EMS disponible!', 'CHAR_CALL911', 8)
	end
end)
