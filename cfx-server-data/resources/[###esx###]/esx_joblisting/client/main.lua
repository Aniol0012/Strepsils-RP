...
function ShowJobListingMenu()
	ESX.TriggerServerCallback('esx_joblisting:getJobsList', function(jobs)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'joblisting', {
			...
			TriggerServerEvent('esx_joblisting:setJob', data.current.job) --¡Tienes un trabajo nuevo!
			-- ESX.ShowNotification(_U('new_job'))
			local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(-1))
			ESX.ShowAdvancedNotification('Trabajo', '~g~Oficina de Empleo', '¡Tienes un nuevo empleo!', mugshotStr, 2) -- Hecho por Aniol0012
...
Citizen.CreateThread(function()
	for i=1, #Config.Zones, 1 do
		...
		SetBlipColour (blip, 2) -- Default 27
