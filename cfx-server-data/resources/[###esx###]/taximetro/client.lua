...
local fareCost = 0.2 --(1.66 = $100 per minute) Cost per second
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          msg = "Presiona F6 para encender el taxímetro"
          TriggerEvent("pNotify:SendNotification", {text = msg , type = "info", layout = "centerRight", queue = "global", theme = "gta", timeout = 8000})
		  --local mugshot, mugshotStr = ESX.Game.GetPedMugshot(PlayerPedId())
		  --ESX.ShowAdvancedNotification('Taxi', '~y~Taxímetro', 'En el menú ~g~F6 s~puedes encender el taxímetro \n~o~L~s~: Reiniciar el servicio. \n~o~K~s~: Contador', mugshotStr, 1)
        ...
        local veh = GetVehiclePedIsIn(ped, false)
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          --[[if IsControlJustReleased(0, 20)  then -- Puesto: ENTER (191) Default: HOME (213)
            TriggerEvent('taxi:toggleDisplay')
			if(meterOpen) then -- Hecho por Aniol0012
				msg = "Has encendido el taxímetro"
			...
			if not(meterOpen) then
				msg = "Has apagado el taxímetro"
				TriggerEvent("pNotify:SendNotification", {text = msg , type = "error", layout = "centerRight", queue = "global", theme = "gta", timeout = 3000})
			end
            Citizen.Wait(100)
          end]]
...
function IsInTaxi()
  ...
  if (name == "Taxi") or (name == "Zentorno") then -- Aqui puedes añadir los coches en los que se puede poner el taximetro.
    return true
  else
    return false
  end
