...
function SendMessage(msg) 
    TriggerEvent("chatMessage", "^0[^5Strepsils RP^0]", {0, 0, 0}, " ^0" .. msg)
end
...
RegisterNUICallback("tpm", function (data)
    local target_id = data.id
    TriggerServerEvent("kc_admin:tpm", target_id, true)
end)

RegisterNUICallback("reparar", function (data)
    local target_id = data.id
    TriggerServerEvent("kc_admin:reparar", target_id, true)
end)
...
--[[
RegisterNetEvent("kc_admin:tpm")
AddEventHandler("kc_admin:tpm", function ()
    local WaypointHandle = GetFirstBlipInfoId(8)

    local waypointCoords = GetBlipInfoIdCoord(WaypointHandle)
	
	SendMessage("Te has ^2teletransportado")
...
AddEventHandler("kc_admin:tpm", function ()
    if godmode then
		...
		AddTextComponentString("Modo dios ~r~desactivado." )
		DrawNotification(false, true)
		godmode = false
	else
		SetEntityInvincible(GetPlayerPed(-1),true)
		SetNotificationTextEntry("STRING")
		AddTextComponentString("Modo dios ~g~activado." )
		DrawNotification(false, true)
		godmode = true
	end
end)

RegisterNetEvent("kc_admin:reparar")
AddEventHandler("kc_admin:reparar", function ()
	SendMessage("Has ^2reparado ^0el vehículo")
