...
RegisterServerEvent("kc_admin:tpm")
AddEventHandler("kc_admin:tpm", function (target_id)
    local Source = source

    if not inArray(getGroup(Source), Permissions.Ban) then
        TriggerClientEvent("kc_admin:send_notify", Source, Lang.Error, Lang.InsufficientPrivileges, "danger")
        TriggerClientEvent("kc_admin:send_message", Source, "^1" .. Lang.InsufficientPrivileges)
        CancelEvent()
        return
    ...
    TriggerClientEvent("kc_admin:tpm", target_id)
    TriggerClientEvent("kc_admin:send_notify", Source, Lang.Success, Lang.Tpm, "success")

end)

RegisterServerEvent("kc_admin:reparar")
AddEventHandler("kc_admin:reparar", function (target_id)
    local Source = source
    ...
    TriggerClientEvent("kc_admin:reparar", target_id)
    TriggerClientEvent("kc_admin:send_notify", Source, Lang.Success, Lang.Reparar, "success")
            ...
            MySQL.Async.fetchAll("SELECT id FROM kc_warns WHERE admin_identifier=@admin_identifier AND identifier=@identifier AND timestamp=@timestamp AND reason=@reason", {["@admin_identifier"] = admin_identifier, ["@identifier"] = user_identifier, ["@timestamp"] = date, ["@reason"] = reason}, function (result)
                ...
                MySQL.Async.fetchAll("SELECT id FROM kc_warns WHERE identifier=@identifier", {["@identifier"] = user_identifier}, function(result)
                    ...
                    if warn_count == Config.WarnWeek then
                        TriggerEvent("kc_admin:ban", Groups.Server, user_id, Lang.WarnAccumulation .. warn_count ,259200) -- Puesto: 3 dias de baneo. /// Default: 1468800. Esto son 408 horas, es decir, 17 dias
                        return
                    end

                    if warn_count == Config.WarnDays then
                        TriggerEvent("kc_admin:ban", Groups.Server, user_id, Lang.WarnAccumulation  .. warn_count, 86400) -- Puesto: 1 dia de baneo. /// Default: 259200 Esto son 72 horas, es decir, 3 dias
                        return
                    end
