TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^2SYSTEM', '^1¡Permisos Insuficientes!' } })
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^2SYSTEM', 'That job does not exist.' } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^2SYSTEM', 'Player not online.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^2SYSTEM', 'Invalid usage.' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^2SYSTEM', '^1¡Permisos Insuficientes!' } })
