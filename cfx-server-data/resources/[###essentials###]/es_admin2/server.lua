...
AddEventHandler('es:incorrectAmountOfArguments', function(source, wantedArguments, passedArguments, user, command)
	if(source == 0)then
		...
		TriggerClientEvent('chat:addMessage', source, {
			args = {"^2SISTEMA", "Incorrect amount of arguments! (" .. passedArguments .. " passed, " .. requiredArguments .. " wanted)"}
		})
	end
end)
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chat:addMessage', Source, {
					args = {"^2SISTEMA", "You do not have permission to do this"}
				})
			end
		end)
	end)
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if t == "group" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chat:addMessage', source, {
						args = {"^2SISTEMA", "Player not found"}
					})
				else
			...
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						...
						TriggerClientEvent('chat:addMessage', Source, {
							args = {"^2SISTEMA", "Permission level of ^2" .. GetPlayerName(tonumber(USER)) .. "^0 has been set to ^2 " .. tostring(GROUP)}
						})
					else
						TriggerClientEvent('chat:addMessage', Source, {
							args = {"^2SISTEMA", "Invalid integer entered"}
						})
					end
				end
			elseif t == "money" then
			...
			elseif t == "bank" then
				...
				TriggerClientEvent('chat:addMessage', Source, {
					args = {"^2SISTEMA", "Superadmin required to do this"}
				})
			end
		end)
	end)
			...
			if args[2] then
				TriggerEvent("es:getPlayerFromId", player, function(user)
					...
					TriggerClientEvent('chat:addMessage', user.get('source'), {
						args = {"^2SISTEMA", "You've been given a role: ^2" .. role}
					})
				end)
			else
		...
		if (player and GetPlayerName(player)) then
			if args[2] then
				TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
					...
					TriggerClientEvent('chat:addMessage', user.get('source'), {
						args = {"^2SISTEMA", "You've been removed a role: ^2" .. role}
					})
				end)
			else
				...
				TriggerEvent("es:getPlayerFromId", player, function(user)
					if(user)then
						...
						TriggerClientEvent('chat:addMessage', player, {
							args = {"^2SISTEMA", "Your money has been set to: ^2^*$" .. money}
						})
					end
				end)
			else
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, {
		args = {"^2SISTEMA", "Level: ^*^2 " .. tostring(user.get('permission_level'))}
	})
	TriggerClientEvent('chat:addMessage', source, {
		args = {"^2SISTEMA", "Group: ^*^2 " .. user.getGroup()}
	})
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget then
						...
						TriggerClientEvent('chat:addMessage', -1, {
							args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been kicked(^2" .. reason .. "^0)"}
						})
						...
						TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "You can not target this person"}})
					end
				end)
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
	end
...
TriggerEvent('es:addGroupCommand', 'noclip', "admin", function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "¡Permisos insuficientes!"} })
...
TriggerEvent('es:addGroupCommand', 'anuncio', "admin", function(source, args, user)
	TriggerClientEvent('chat:addMessage', -1, {
		args = {"^2 ANUNCIO ADMINISTRATIVO", table.concat(args, " ")}
	})
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "¡Permisos insuficientes!"} })
end, {help = "Mensaje de administrador", params = {{name = "announcement", help = "Escribe el mensaje para anunciar"}}})
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^2SISTEMA", "You have been " .. state .. " by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been " .. state} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^2SISTEMA", "You have brought by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been brought"} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^2SISTEMA", "You have slapped by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped"} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
		...
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chat:addMessage', player, { args = {"^2SISTEMA", "You have been teleported to by ^2" .. GetPlayerName(source)} })
					TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Teleported to player ^2" .. GetPlayerName(player) .. ""} })
				end
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
...
TriggerEvent('es:addCommand', 'die', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "You killed yourself"} })
end, {help = "Suicide"})
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^2SISTEMA", "You have been killed by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been killed."} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Player ^2" .. GetPlayerName(player) .. "^0 has been crashed."} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^2SISTEMA", "Incorrect player ID"}})
		end
