	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if false then
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 5.5 then
...
if Config.EnableHUD then
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsPedInAnyVehicle(ped) and not (Config.RemoveHUDForBlacklistedVehicle and inBlacklisted) and IsDriver() then
				local vehicle = GetVehiclePedIsIn(ped)
	...
	Citizen.CreateThread(function()
		while true do
			if displayHud then --fuelVisible
				--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
				--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)
				DrawAdvancedText(0.24500 - x, 0.8165 - y, 0.005, 0.0028, 0.5, fuel .. "%", 255, 255, 255, 255, 6, 1)
				DrawAdvancedText(0.20500 - x, 0.8165 - y, 0.005, 0.0028, 0.5, "Gasolina:", 255, 255, 255, 255, 6, 1)
			else
				Citizen.Wait(750)
			end
