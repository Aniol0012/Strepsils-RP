...
AddEventHandler('gln_oponazapasowa:uzyj', function()
	ESX.ShowHelpNotification('~r~¡Acércate a la rueda dañada!')
	animacja()
end)
		...
		if vehicle ~= 0 then
			...
			if closestTire ~= nil then
				if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
					...
					Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, "~r~¡Este neumático necesita una reparación!")
				end
			end
		end
	end
		...
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('gln_oponazapasowa:usunOpone')
				...
				Citizen.CreateThread(function()
					...
					CurrentAction = 'opona' -- Si no funciona cambiar esto por: rueda

					TriggerEvent("pNotify:SendNotification", {text = "¡Estás cambiando el neumático!", type = "info", timeout = 5000, layout = "centerRight"})
					...
					if CurrentAction ~= nil then
						...
						if closestTire ~= nil then
							...
							TriggerEvent("pNotify:SendNotification", {text = "¡Has terminado de cambiar el neumático!", type = "success", timeout = 2000, layout = "centerRight"})
						end
					end
