...
Citizen.CreateThread(function()
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded and not IsPedInAnyPlane(MyPed) and not IsPedInAnyHeli(MyPed) and not IsPedInAnyBoat(MyPed) then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Pulsa [~g~E~w~] para empujar el vehiculo', 0.4)
                end
     

            if IsControlPressed(0, Keys["E"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded and not IsPedInAnyPlane(MyPed) and not IsPedInAnyHeli(MyPed) and not IsPedInAnyBoat(MyPed) then
