...
function RefreshSkills()
	for v,i in pairs(mySkills) do
		...
		elseif i == 'transport' or i == 'bus' or i == 'taxi' or i == 'mecano' or i == 'pilot' or i == 'trucker' then
...
function openJobsMenu()
	local elements = {
		...
		label = "Mecánico" ,
		title = "Mecánico",
		description = " Para poder acudir a llamadas técnicas y hacer cosas de mecánicos tienes que estudiar esto.",
		necesarySp = 8,
		money = 1200,
		skillName = "mecano",
		levelNeeded = nil,
...
local university = {x = -136.8, y = 6292.31, z = 35.5}
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(university.x, university.y, university.z - 1, x,y,z) < 1.5 then
			drawTxt("PULSA E PARA ABRIR LA UNIVERSIDAD",4, 1, 0.5, 0.835, 0.85,255,255,255,255)
...
local blips = {
	{title="Universidad", colour=2, id=498, x = -136.8, y = 6292.0, z = 34.5}
}
Citizen.CreateThread(function()
      blip = AddBlipForCoord(blips.x, blips.y, blips.z)
