...
Citizen.CreateThread(function()
	while true do
        ...
        if IsPedInAnyVehicle(MyPed, false) and permito then
...
--[[
RegisterNetEvent('strepsils_cuentakilometros2')
AddEventHandler('strepsils_cuentakilometros2', function()
	permito = not permito
end)
--]]
...
RegisterNetEvent('strepsils_abrir_cuentakilometros2')
AddEventHandler('strepsils_abrir_cuentakilometros2', function()
	if not permito then
		TriggerEvent('strepsils_cerrar_cuentakilometros_carreras')
		permito = true
	end
...
RegisterNetEvent('strepsils_cerrar_cuentakilometros2')
AddEventHandler('strepsils_cerrar_cuentakilometros2', function()
	if permito then
		permito = false
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(player, false) and permito then
			-- Vehicle Speed
			...
			if(vehicleClass ~= 13 and vehicleClass ~= 15 and vehicleClass ~= 16)then
				...
				if vehicleCruiser == 'on' then
					...
					if(kmhSpeed > 20)then
						...
						TriggerEvent('strepsils_controldevelocidad')
					end
				end
			end
        end
