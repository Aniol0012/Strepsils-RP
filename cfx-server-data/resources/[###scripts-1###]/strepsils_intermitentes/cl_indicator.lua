...
function IsDriver () -- Piloto [Hecho por Aniol0012]
...
Citizen.CreateThread(function()
	while true do
		...
		if ped ~= nil and GetPedInVehicleSeat(ped, -1) == GetPlayerPed(-1) then
			if IsControlJustPressed(1, 174) and IsDriver() then
				indicatorTime = 0
				...
				TriggerServerEvent("asser:syncIndicator", indicator)
				TriggerEvent("asser:setIndicator", indicator)
			elseif IsControlJustPressed(1, 175) and IsDriver() then
				indicatorTime = 0
				...
				TriggerServerEvent("asser:syncIndicator", indicator)
				TriggerEvent("asser:setIndicator", indicator)
			elseif IsControlJustPressed(1, 173) and IsDriver() then
				indicatorTime = 0
