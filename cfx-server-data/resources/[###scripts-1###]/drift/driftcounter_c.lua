...
Citizen.CreateThread( function()
	...
	function angle(veh)
		...
		if GetEntitySpeed(veh)* 3.6 < 30 or GetVehicleCurrentGear(veh) == 0 then return 0,modV end --Velociadad superior a 30 km/h
	...
	while true do
		...
		if not IsPedDeadOrDying(PlayerPed, 1) and GetVehiclePedIsUsing(PlayerPed) and GetPedInVehicleSeat(GetVehiclePedIsUsing(PlayerPed), -1) == PlayerPed and IsVehicleOnAllWheels(GetVehiclePedIsUsing(PlayerPed)) and not IsPedInFlyingVehicle(PlayerPed) then
			...
			if not tempBool and score ~= 0 then
				...
				TriggerServerEvent("SaveScore2", GetPlayerServerId(PlayerId()), score) 
				score = 0
			end
			if angle ~= 0 then
				if score == 0 then
...
local driftaniol = true

--[[
ToggleDrift = function()
	driftaniol = not driftaniol
end

RegisterNetEvent('strepsils_drift')
AddEventHandler('strepsils_drift', function()
	ToggleDrift()
end)
]]
