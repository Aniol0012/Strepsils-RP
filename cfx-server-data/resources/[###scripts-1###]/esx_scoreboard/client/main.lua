...
Citizen.CreateThread(function()
	...
	SendNUIMessage({
		...
		maxPlayers = GetConvarInt('sv_maxclients', 64), -- Aqui se ponen los jugadores maximos
		uptime = 'unknown',
		playTime = '00h 00m'
	})
end)
...
function UpdatePlayerTable(connectedPlayers)
	...
	for k,v in pairs(connectedPlayers) do
		...
		elseif v.job == 'mechanic' then
			mechanic = mechanic + 1
		elseif v.job == 'admin' then
			cardealer = cardealer + 1
		elseif v.job == 'mafia1' or v.job == 'mafia2' or v.job == 'mafia3' or v.job == 'mafia4' then
			estate = estate + 1
		end
	end
...
RegisterNetEvent('strepsils_scoreboardtoggle')
AddEventHandler("strepsils_scoreboardtoggle", function()
	SendNUIMessage({
		action = 'toggle'
	})
end)

RegisterNetEvent('strepsils_scoreboardclose')
AddEventHandler("strepsils_scoreboardclose", function()
	SendNUIMessage({
		action = 'close'
	})
end)
