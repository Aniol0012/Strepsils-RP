RegisterCommand("reparar", function(source, args)   -- Puedes hacer el comando que quieras cambiando el comando que dice 'reparar'.
	TriggerServerEvent('pagarmecano:checkmoney')
end, false)


RegisterCommand("repair", function(source, args)   -- Puedes hacer el comando que quieras cambiando el comando que dice 'repair'.
	TriggerServerEvent('pagarmecano:checkmoney')
end, false)
...
function SpawnVehicle(x, y, z, vehhash, driverhash)                                                    
    ...
    if found and HasModelLoaded(vehhash) and HasModelLoaded(vehhash) then
        ...
        SetBlipColour(mechBlip, 22) -- Gris
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Mecánico avisado", "~g~Ayuda enviada hasta el punto en el que estabas averiado.")
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "~g~¡Vehículo reparado!" , "")
	...
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "~b~¡Que tengas un buen dia!" , "")
	
	
	Citizen.Wait(5000) -- Me espero 5 sec
	TriggerServerEvent('pagarmecano:cobrar')
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "" , "Has pagado ~r~2000€~w~ de tu cartera")
end
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("Debes estar cerca del vehículo para pedir ~o~Asistencia en carretera~w~.")
    end
end
...
AddEventHandler('pagarmecano:success', function (price)
	...
	--ShowNotification("Has pagado el precio por adelantado!~r~-$" .. price .. "~s~.En breves vendrá un mecánico!")
	--ShowAdvancedNotification(companyIcon, "Mecánico", "" , "Se ha enviado tu mensaje, en breves vendrá un mecánico.")
end)

RegisterNetEvent('pagarmecano:notenoughmoney')
AddEventHandler('pagarmecano:notenoughmoney', function (moneyleft)
	ShowNotification("~r~No tienes suficiente dinero, necesitas ~g~" .. moneyleft .. "€~s~.")
end)
...
AddEventHandler('pagarmecano:free', function ()
	...
	ShowNotification("Your vehicle was ~y~cleaned~s~ for free!")
end)
-------------------
