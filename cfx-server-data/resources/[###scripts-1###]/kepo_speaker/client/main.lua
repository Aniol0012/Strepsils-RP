...
RegisterNetEvent('strepsils_cerrar_altavoz')
AddEventHandler('strepsils_cerrar_altavoz', function()
	SetNuiFocus(false, false)
end)
...
RegisterNetEvent("kepo_speaker:revisar")
AddEventHandler("kepo_speaker:revisar", function()
	if not IsPedInAnyVehicle(PlayerPedId(), true) then
		TriggerEvent("kepo_speaker:place")
	else
		local mugshot, mugshotStr = ESX.Game.GetPedMugshot(PlayerPedId())
		ESX.ShowAdvancedNotification('Tú', '', '~r~No puedes estar dentro de un vehículo para colocar un altavoz.', mugshotStr, 1)
