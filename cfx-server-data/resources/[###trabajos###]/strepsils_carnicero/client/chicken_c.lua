...
LoadDict('anim@heists@box_carry@')
	...
	TriggerEvent('notification', 'You stored the chickens in the vehicle.', 1)
...
if(count > 0) then
	...
	exports['taskbar']:taskBar(7500, 'שם את התרנגול בקופסה')
	TriggerServerEvent("tostKurczaki:przerob",2)
	--ESX.ShowNotification("~y~Pakuj kurczaki dalej lub udaj się do pojazdu.")
	TriggerEvent('notification', 'Keep packing the chicken or go to the vehicle and store it.', 1)
	ClearPedTasks(GetPlayerPed(-1))
	DeleteEntity(karton)
	DeleteEntity(mieso)
else
...
SetEntityRotation(kurczak,90.0, 0.0, -45.0, 1,true)
exports['taskbar']:taskBar(10000, 'חותך את התרנגול')
end
--ESX.ShowNotification("~g~Poćwiartowałeś kurczaka.")
TriggerEvent('notification', 'You slaughtered a chicken!', 1)
FreezeEntityPosition(GetPlayerPed(-1),false)
...
TriggerEvent('notification', 'You dont have any chickens!', 2)
end
end
...
LoadDict('anim@heists@box_carry@')
	...
	dajdoAuta = false
	--ESX.ShowNotification("~y~Wkładasz kurczaki do samochodu.")
	TriggerEvent('notification', 'You put the chicken in the vehicle!', 2)
	LoadDict('anim@heists@narcotics@trash')
	TaskPlayAnim(GetPlayerPed(-1), 'anim@heists@narcotics@trash', "throw_a", 3.0, -8, -1, 63, 0, 0, 0, 0 )
	Citizen.Wait(900)
	ClearPedTasks(GetPlayerPed(-1))
...
if akcja == true then
	...
	TriggerEvent('notification', 'Take the chickens to a vehicle!', 1)
	TepnijWyjscie()
	end
...
function ZostalZlapany()
	...
	if szansaZlapania <= 60 then
			TriggerEvent('notification', 'You managed to catch 1 chicken!', 1)
			if Zlapany1 == 1 then
				DeleteEntity(kurczak1)
				Zlapany1 = 0
		...
		TriggerEvent('notification', 'The chicken escaped your arms!', 2)
	end
end
