...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'sadownik' and not onDuty then
	...
	while true do
		...
		if onDuty then
          ...
          if CurrentAction and not IsDead then
            ...
            if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'sadownik' then
...
if onDuty == true then
	if not ESX.Game.IsSpawnPointClear(Config.Zones.Job1.Pos, 20.0) then
		TriggerEvent("pNotify:SendNotification", {text = "Ktoś pozostawił pojazd na terenie sadu, ZBIORY WSTRZYMANE!", type = "error", queue = "global", timeout = 5000, layout = "centerLeft"})
		Citizen.Wait(2000)
		HasAlreadyEnteredMarker, LastZone = true, currentZone
		TriggerEvent('esx_orchard:hasEnteredMarker', currentZone)
	else
	if IsPedInAnyVehicle(PlayerPedId()) then
		...
		if tree ~= 0 then
			if isHoldingOwoce == true then
				TriggerEvent("pNotify:SendNotification", {text = "Najpierw odstaw owoce do strefy pakowania", type = "error", queue = "global", timeout = 5000, layout = "centerLeft"})
			else
			animacjazbierania()
			FreezeEntityPosition(PlayerPedId(), true)
			procent(150)
...
AddEventHandler('sadownik:niemasz', function()
	TriggerEvent("pNotify:SendNotification", {text = "Nie posiadasz odpowiedniej ilosci owocow.", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"})
	ClearPedSecondaryTask(PlayerPedId())
end)
