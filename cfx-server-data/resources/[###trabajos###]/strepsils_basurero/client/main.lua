                        ...
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ESX.TriggerServerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        exports['mythic_notify']:DoHudText('inform', 'You have received your € 1000 deposit back!')
                                    else
                                        exports['mythic_notify']:DoHudText('inform', 'You have not paid a deposit on this vehicle..')
                                    end
                                end)
                            end
                        else
                                ...
                                ESX.TriggerServerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        spawnvehicle("trash2", function(veh)
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            exports['mythic_notify']:DoHudText('inform', 'You have paid a € 1000 deposit!')
                                            exports['mythic_notify']:DoHudText('inform', 'You have started working, location is indicated on your GPS!')
                                        end, coords, true)
                                    else
                                        exports['mythic_notify']:DoHudText('inform', 'You do not have enough money for the deposit .. Deposit costs are €1000')    
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        exports['mythic_notify']:DoHudText('inform', 'All garbage bags are done, proceed to the next location!')
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        exports['mythic_notify']:DoHudText('inform', 'You are done working! Go back to the dump.')
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        exports['mythic_notify']:DoHudText('inform', 'There is still '..AmountOfBags..' bags over!')
                                                    else
                                                        exports['mythic_notify']:DoHudText('inform', 'There is still '..AmountOfBags..' bag over!')
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
