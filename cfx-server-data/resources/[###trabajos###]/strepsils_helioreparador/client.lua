...
function Cut(Id, Object, Pped, treeCoords)
	...
	local prop = CreateObject(GetHashKey("prop_tool_wrench"), x, y, z + 0.2, true, true, true)
						 ...
						 TriggerEvent("mythic_progbar:client:progress", {
							...
							controlDisables = {
								...
								if not status then
									...
									-- Ne touchez pas ici je regle les probleme de tic serveur
	...
	TaskPlayAnim(Pped, "anim@gangops@facility@servers@bodysearch@", "player_search", 3.0, 3.0, -1, 50, 0, false, false, false)
...
function SpawnTree()
	...
	for k,zone in pairs(config.spawn) do
		...
		ESX.Game.SpawnLocalObject('xm_prop_base_work_station_01', zone.coords, function(obj)
...
function SpawnNewTree(treeCoords)
	...
	ESX.Game.SpawnLocalObject('xm_prop_base_work_station_01', treeCoords, function(obj)
				...
				ESX.TriggerServerCallback('caruby_lumberjack:canCUT', function(canCUT)
					...
					if canCUT then
							...
							if not cutting then
									...
									ESX.ShowHelpNotification("Appuyez ~INPUT_CONTEXT~ pour Reparer l Eolienne")
						...
						ESX.ShowHelpNotification("Vous devez acheter un outil de réparation !")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.dealer.coords, true) < 1.5 then
			...
			ESX.ShowHelpNotification("Appuyez ~INPUT_CONTEXT~ pour ~g~Touchez votre Salaire")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if cankeep then
			...
			ESX.ShowHelpNotification("Appuyez ~INPUT_CONTEXT~ pour Rallumer l Eolienne")				
...
function OpenWoodShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wood_shop', {
		...
		title    = 'Syndicat des Travailleurs Eolien',
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			...
			if not isProcessing then
				...
				ESX.ShowHelpNotification("Appuyez ~INPUT_CONTEXT~ pour ~g~Recycler les morceaux d Eolienne")
...
--- Si vous rencontrez des erreurs veuillez me mp Ozne#4870