...
Citizen.CreateThread(function()
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      --if beltOn then DisableControlAction(0, 57) end -- Esto es para que si estas con el cinturon puesto, te desactiva el F10.
      ...
      if IsControlJustReleased(0, 47) and GetLastInputMethod(0) then -- (47 para G)
        beltOn = not beltOn 
        if beltOn then 
		  ...
		  TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0.5, 'unbuckle', 0.5)
  ...
  Citizen.Wait(0)
    if not beltOn and wasInCar == true then
    	...
    	--------- Here you can change sounds, when seatbelt is off ------------- Here -----------
                TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0, 'seatbelt', 0.15)
                Citizen.Wait(13000)
    end
