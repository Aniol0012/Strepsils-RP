...
window.APP = {
  ...
  watch: {
	...
	messagesP() {
      ...
      const messagesObj = this.$refs.messagesP;
      this.$nextTick(() => {
        messagesObj.scrollTop = messagesObj.scrollHeight;
      });
    },
	...
	messagesM() {
      ...
      const messagesObj = this.$refs.messagesM;
      this.$nextTick(() => {
        messagesObj.scrollTop = messagesObj.scrollHeight;
      });
    },
  ...
  methods: {
    ...
    ON_MESSAGEP({ message }) {
      this.messagesP.push(message);
    },
    ON_MESSAGEM({ message }) {
      this.messagesM.push(message);
    },
    ON_MESSAGEG({ message }) {
    ...
    send(e) {
      ...
      if(msg !== '') {
        if (channel == 'global') {
          ...
          this.oldMessagesIndex = -1;
          this.hideInput();
        } else if (channel == 'police') {
          $.post('http://chat/chatResultP', JSON.stringify({
            message: getEmojiChars(msg),
          }));
          this.oldMessages.unshift(msg);
          msg = '';
          this.oldMessagesIndex = -1;
          this.hideInput();
        } else if (channel == 'medic') {
          $.post('http://chat/chatResultM', JSON.stringify({
            message: getEmojiChars(msg),
          }));
          this.oldMessages.unshift(msg);
          msg = '';
...
$(function()
{
	...
	$(".emojiPanel").on('click', function() {
		//console.log(1);

		//var txtToAdd = $(".emojiPanel").attr("value");
...
function openPage(pageName, elmnt, evt) {
  ...
  elmnt.style.color = "aliceblue";
  elmnt.style.backgroundColor = "rgba(0, 0, 0, 0.432)";
...
document.getElementById("defaultOpen").click();
	...
	function getEmojiChars(text) {
		var emojRange = [
		  [':)', '🙂'], ['<3', '❤'], [':D', '😀'], [':(', '😔'], ['XD', '😆'], ['xd', '😆'], ['mierda', '💩'], ['subnormal', '*********'], ['hijo de puta', '*********'], ['Hijo de puta', '*********'], ['200IQ', ':45:'], ['200iq', ':45:'], ['200 de IQ', ':45:'], ['200 IQ', ':45:'], ['Logo strepsils rp', ':1:'], [':joy:', '😂']
		];
		var finalText = text;
